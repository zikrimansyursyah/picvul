...
export default function Login({ setForm, setAlert, setLoadState }) {
  ...
  function setUserSession(data) {
    ...
    const sign = jwt.sign(data.data, "lumba-lumba");
    setCookie("user_session", sign, 1);
  }

  const handleLogin = async (e) => {
    setLoadState(true);
    const response = await authLogin(formik.values);
    setLoadState(false);
    if (response.status === 200) {
      setUserSession(response.data);
      router.push("/");
    } else if (!response.data || response.toString().split(" ")[6]) {
      setAlert({
        type: false,
